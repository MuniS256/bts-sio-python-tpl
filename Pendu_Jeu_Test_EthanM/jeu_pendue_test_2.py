...
class JeuPendu:
    def __init__(self):
        ...
        pygame.display.set_caption("Le Pendu POO")
        self.clock = pygame.time.Clock()
        self.etat = "MENU"

        self.mots_difficulte = {
            "FACILE": ["CHAT", "FEU", "SOLEIL", "POMME", "LUNE"],
            "MOYEN": ["PYTHON", "SERVEUR", "BOUCLE", "RESEAU", "CLAVIER"],
            "DIFFICILE": ["ALGORITHME", "INDENTATION", "CRYPTOGRAPHIE", "PROCESSEUR"]
        }
        ...
        self.btn_quitter_jeu = Bouton("QUITTER LE JEU", LARGEUR_ECRAN//2, 480, 220, 45, GRIS_CLAIR)

        # Boutons Fin de partie (Victoire ou Défaite)
    ...
    def boucle(self):
        while True:
            ...
            if self.etat == "MENU":
                ...
                self.tous_sprites.draw(self.ecran)
                msg = pygame.font.SysFont("Arial", 35, bold=True).render(f"DOMMAGE ! LE MOT ÉTAIT : {self.mot}", True, NOIR_DOUX)
                ...
                if self.btn_rejouer.est_clique(pos_souris, clic): self.lancer_partie(self.niveau_actuel)
                if self.btn_retour_menu.est_clique(pos_souris, clic): self.etat = "MENU"

            elif self.etat == "VICTOIRE":
                self.tous_sprites.draw(self.ecran)
                msg = pygame.font.SysFont("Arial", 40, bold=True).render("BRAVO, TU AS TROUVÉ !", True, (50, 150, 50))
                self.ecran.blit(msg, msg.get_rect(center=(LARGEUR_ECRAN//2, 380)))
                self.btn_rejouer.dessiner(self.ecran)
                self.btn_retour_menu.dessiner(self.ecran)
                if self.btn_rejouer.est_clique(pos_souris, clic): self.lancer_partie(self.niveau_actuel)
